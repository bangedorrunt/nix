...
;;; core/autoload/keybind.el --- defaults for Doom's keybinds -*- lexical-binding: t; -*-
;;; Commentary:
;;
...
(defun doom/escape (&optional interactive)
  ...
  (cond
   ...
   ((and (boundp 'corfu--frame) (frame-live-p corfu--frame))
    (when interactive
      (setq this-command 'corfu-quit))
    (corfu-quit))
   ...
   (defining-kbd-macro
    ...
    (cancel-kbd-macro-events))   ;; Back to the default
...
;;;###autoload
(defvar doom-localleader-alt-key "M-SPC ,"
          ...
          (let* ((udef (cdr-safe (doom-unquote def)))
            ...
            (unless (eq bdef :ignore)
              (push `(define-key doom-leader-map (general--kbd ,key)
                                 ,bdef)
                    forms))
            (when-let (desc (cadr (memq :wk udef)))
              (prependq!
  ...
  (when desc
    (let (unquoted)
      (cond ((and (listp def)
             ...
             (setq def (list 'quote (plist-put unquoted :wk desc))))
            ((setq def (cons 'list
                             (if (and (equal key "")
                                      (null def))
                                 `(:ignore t :wk ,desc)
