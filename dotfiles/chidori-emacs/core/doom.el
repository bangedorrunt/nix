;;; core/doom.el --- Doom's core standard library -*- lexical-binding: t; -*-
;;; Commentary:
;;; Code:
...
(defvar IS-BSD (or IS-MAC (eq system-type 'berkeley-unix)))
(defvar IS-WINDOWS (memq system-type '(cygwin windows-nt ms-dos)))
  ...
  (condition-case-unless-debug e
    ...
    (error
     ...
     (signal (cond ((not (and path (featurep 'doom)))
                    'error)
                   ((file-in-directory-p path chidori-core-dir)
                    'doom-core-error)
                   ((file-in-directory-p path chidori-lisp-dir)
                    'doom-module-error)
                   ('doom-error))
             (list path e)))))
...
Can also load Doom's subfeatures, e.g. (doom-require 'doom-lib 'files)"
  (let ((subfeature (if (symbolp filename) filename)))
    (or (featurep feature subfeature)
        (doom-load
         (if subfeature
             (file-name-concat chidori-doom-dir (symbol-name filename))
           (symbol-name feature))
         noerror))))

(defun doom-requires (&rest pkgs)
  "Load PKGS"
  (mapc (lambda (pkg) (require `,pkg)) pkgs))

(defun doom-has-modules-p ()
...
- Supports compound package statements (see :or/:any and :and/:all above).
...
This has the same signature as `defadvice!' an exists as an easy undefiner when testing advice (when combined with `rotate-text').
